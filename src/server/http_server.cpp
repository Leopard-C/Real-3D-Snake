...
void HttpServer::add_mapping(std::string path, MethodHandler handler, HttpMethod method) {
    Resource res = {method, handler, NULL, NULL};
    (*_resource_map)[path] = res;
}

void HttpServer::add_mapping(std::string path, JsonHandler handler, HttpMethod method) {
...
void HttpServer::remove_mapping(const std::string& path) {
    auto findIter = _resource_map->find(path);
    if (findIter != _resource_map->end()) {
        _resource_map->erase(findIter);
    }
}

void HttpServer::remove_all_mappings() {
    std::map<std::string, Resource>().swap(*_resource_map);
}
    ...
    public:
        ...
        int action(Request &req, Json::Value &root) {
            ...
            for (; it != eclients.end(); it++) {
                root["clients"][i]["id"] = Json::Int64((*it).first);
                root["clients"][i]["fd"] = (*it).second->fd;
                root["clients"][i]["idle"] = Json::Int64((long long)time(NULL) - (*it).second->_last_interact_time);
                i++;
            }
            return 0;
        }
...
int HttpEpollWatcher::handle_request(Request &req, Response &res) {
    ...
    if (resource.json_handler) {
    //if (resource.json_ptr != NULL) {
        Json::Value root;
        resource.json_handler(req, root);
        //resource.json_ptr(req, root);
        res.set_body(root);
    } else if (resource.method_handler != NULL) {
    //} else if (resource.handler_ptr != NULL) {
        resource.method_handler(req, res);
